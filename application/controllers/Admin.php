...
class Admin extends CI_Controller
{
	...
	function student_add()
	{
		...
		$page_data['page_title'] = get_phrase('Ajouter un élève');
		$this->load->view('backend/index', $page_data);
	}
	...
	function student_bulk_add($param1 = '')
	{
		...
		if ($param1 == 'import_excel')
		{
           ...
           // ini_set('error_reporting', E_ALL);
	    	...
	    	if ( $xlsx = SimpleXLSX::parse( 'uploads/student_import.xlsx' ) ) {
	   foreach ( $xlsx->rows() as $r => $row ) {
		...
		foreach ( $row as $c => $cell ) {
            ...
            if ($f == 0)	    $data['name']			=	$cell;
			else if ($f == 1)	$data['birthday']		=	$cell;
			else if ($f == 2)	$data['sex']		    =	$cell;
			else if ($f == 3)	$data['address']		=	$cell;
			else if ($f == 4)	$data['phone']			=	$cell;
			else if ($f == 5)	$data['email']			=	$cell;
			else if ($f == 6)	$data['password']		=	hash('sha256', $cell); 
			else if ($f == 7)	$data['roll']			=	$cell;
            else if ($f == 8)	$data['section_id']	    =	$cell;
            $f++;
        ...
        if ($query->num_rows() > 0) {
            ...
            echo "<script>alert('L\'email existe déjà : " . $email . "');</script>";
        } else {
...
$this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
	   ...
	   // redirect(base_url() . 'index.php?admin/student_information/' . $this->input->post('class_id'), 'refresh');
		...
		$page_data['page_title'] = get_phrase('Add Bulk Student');
		$this->load->view('backend/index', $page_data);
	}
	...
	function student_information($class_id = '')
	{
		...
		$page_data['page_title'] 	= get_phrase('Informations des élèves'). " - ".get_phrase('class')." : ".
	...
	function student_marksheet($class_id = '')
	{
		...
		$page_data['page_title'] 	= get_phrase('Feuille de notes des élèves'). " - ".get_phrase('class')." : ".
											$this->crud_model->get_class_name($class_id);
		$page_data['class_id'] 	= $class_id;
		$this->load->view('backend/index', $page_data);
	}
    ...
    function exam_marks_sms($param1 = '' , $param2 = '')
    {
        ...
        $page_data['page_title'] = get_phrase('Send Marks by SMS');
        $this->load->view('backend/index', $page_data);
    }
    ...
    function marks($exam_id = '', $class_id = '', $subject_id = '')
    {
        ...
        $page_data['page_title'] = get_phrase('Manage Exam Marks');
        $this->load->view('backend/index', $page_data);
    }
	...
	function manage_attendance($date='',$month='',$year='',$class_id='')
	{
        ...
        $page_data['page_title'] =	'Présences quotidiennes';
		$this->load->view('backend/index', $page_data);
	}
    ...
    function invoice($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        $page_data['page_title'] = 'Gestion des factures/paiements';
    ...
    function expense_category($param1 = '' , $param2 = '')
    {
        ...
        $page_data['page_title'] = 'Catégorie de dépenses';
        $this->load->view('backend/index', $page_data);
    }
    ...
    function book($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        $page_data['page_title'] = 'Gérer les livres de la bibliothèque';
        $this->load->view('backend/index', $page_data);
    ...
    function noticeboard($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        $page_data['page_title'] = 'Tableau d’affichage';
